...
允许通过 `okagent.qwen_causal_proxy.QwenCausalProxy` 在 CPU 本地运行小 Qwen，
但它的输出只是 proxy 预测，不能写入 SemanticOperator 标签缓存或冒充真实标签。
...
- 主 proxy 必须是 `okagent.qwen_causal_proxy.QwenCausalProxy`：直接在 CPU 部署
  `Qwen/Qwen3-0.6B` 因果语言模型，由 Qwen 自身比较 A=不匹配、B=匹配的下一 token logits 并输出匹配概率。
  这不是 Qwen embedding + LogisticRegression；不得另训 LR、SVM、TF-IDF 分类器冒充主 proxy。
- 用训练标签调用 `proxy.fit(labels)` 选择少量、平衡的 in-context demonstrations；验证标签只用于选满足 recall>=0.9 的最大阈值，不能进入 demonstrations。
  `score_ids` 会按模型、岗位、示例和数据库缓存分数；通过 `OKAGENT_QWEN_CAUSAL_CACHE` 在 baseline/LO-PH 间复用完全相同配置的结果。
  先对 2 人冒烟测试，再按小 batch 在 CPU 评分全库；不得擅自换成 4B/8B，也不得静默回退已有 embedding。Qwen 加载失败应使本轮 proxy 明确失败。
- 已有 2048 维 embedding 或 Qwen embedding + LR 只能作为单列对照实验，不能作为这轮“小 Qwen 本体作为 proxy”的结果。
...
- 保存 `backend=qwen_causal_lm`、模型名、上下文长度、demonstration ID/标签、cache 路径和阈值，确保部署可复现；不能读取历史 llm_pass。
"""
...
```
小 Qwen proxy 的固定调用方式：
```python
from okagent.qwen_causal_proxy import QwenCausalProxy
proxy = QwenCausalProxy.from_workspace('.')
proxy.fit(train_labels)  # dict[candidate_id] = 0/1；只传训练集，不传验证集
scores = proxy.score_ids(ids)  # Qwen3-0.6B 本体给出的 P(B=匹配)
proxy.save('output/proxy.pkl', threshold=threshold)
proxy, threshold = QwenCausalProxy.load('output/proxy.pkl', '.')
```
...
- `proxy.pkl`：用 `QwenCausalProxy.save(...)` 保存的小 Qwen proxy 配置、demonstrations 和阈值（内容为 JSON，不包含模型权重）。
