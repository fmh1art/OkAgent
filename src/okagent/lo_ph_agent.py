...
CONTRACTS = {
    ...
    "proxy": "输入 train，应提供 validation；严格按当前 proxy variant 实现模型、缓存和阈值。control/paper_skill 使用 Qwen direct；cascade/combined 使用 LR recall gate→Qwen precision verifier。validation 只用于模型与阈值选择。返回 artifacts.model。",
...
严格执行本 system prompt 末尾的 proxy skill 与实验变体；control/paper_skill 的 backend 必须是 `qwen_direct`，cascade/combined 的 backend 必须是 `lr_qwen_cascade`，不得在两者之间静默切换。
Qwen 必须在本次 implementation.py 中用 transformers/torch 加载，把岗位要求和简历直接交给 Qwen 本体并比较 A/B 下一 token logits；不得改成 Qwen embedding + LR 或远程 Qwen API。纯 CPU 加载使用 `AutoModelForCausalLM.from_pretrained(model_name, torch_dtype=torch.float32).to('cpu').eval()`，禁止 `device_map`、`low_cpu_mem_usage=True` 和 `torch.set_default_device`。
tokenizer 使用 `truncation=True,max_length=1024`；Qwen 分数按模型名、岗位、文本、prompt、截断配置和 demonstrations 哈希增量缓存，重跑不得重复计算。Qwen 失败时算子必须失败，不得伪造结果。不要把 Qwen 权重复制进工作区。
主动采样从 SemanticOperator('.').cached_labels() 的键排除全部已标注 ID，不能只用启动时的训练文件；读取行数组时提取 row['candidate_id']，不能把整个字典转成字符串当 ID。
Deploy 根据 artifact backend 恢复 Qwen direct 或 LR→Qwen cascade；验证和全库必须使用完全相同的评分路径并优先复用分数缓存，自检同一 ID 的分数一致。
