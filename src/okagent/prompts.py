...
- 2000 次预算的本次实验不得在最初 100 条训练标签后直接部署：先用当前 Qwen 对训练池评分，至少再进行一轮预测少数类主动采样与在线重训；最终累计训练标签须达到总预算的 75%（2000 时为 1500），验证标签另计。训练器只需用 `fit --train <每轮路径> --validation <已标注验证表> --output-dir <产物目录>`，不要重新实现训练、评分或阈值函数。
- 训练、主动采样、验证和部署必须调用训练器中的同一文本构造、tokenizer、最大长度和 score 函数。Qwen 权重或 adapter 加载、训练、评分失败时实验必须失败，不能改用其他模型。
- 读取每位候选人的全部 `segment='unstructured'` 文本；超过单次输入长度时按 token 分块覆盖全文，训练时每块继承候选人标签并按候选人均衡权重，评分时聚合所有块。禁止只取前 512/1024 token。旧的截断式 adapter 与分数缓存不得复用，必须重新训练和校准。
