...
严格执行本 system prompt 末尾的 proxy skill 与实验变体；control/paper_skill 的 backend 必须是 `qwen_direct`，cascade/combined 的 backend 必须是 `lr_qwen_cascade`，paper_skill_lr 的 backend 必须是 `embedding_lr_paper_skill`，不得静默切换。
