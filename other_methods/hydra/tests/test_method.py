...
def test_qwen_hiring_backend_supplies_query_vector(tmp_path, monkeypatch):
    raw = tmp_path / "raw"
    descriptions = raw / "job-description-20260722"
    databases = raw / "job-candidate-embedding-20260722"
    descriptions.mkdir(parents=True)
    ...
    config_path.write_text(json.dumps(dict(raw_root=str(raw), job="job01", as_of="2026-07-22", max_calls=20)))
    run_dir = tmp_path / "run"
    prepare(config_path, run_dir)

    import okagent.qwen_proxy as qwen_proxy
    monkeypatch.setattr(qwen_proxy, "load_qwen_features", lambda *args, **kwargs: (
        [str(i) for i in range(8)], np.array([[float(i - 4), 1.0] for i in range(8)]),
        np.array([1.0, 0.0]), {"backend": "qwen", "cached": True}))
    result = run_hiring(run_dir, feature_backend="qwen", config=Config(
        sample_size=4, step=2, calibration_sample_size=8, adaptive=False, max_calls=20))
    assert result["summary"]["initialization"] == "similarity"
    assert result["summary"]["features"]["backend"] == "qwen"
