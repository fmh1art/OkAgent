...
CONTRACTS = {
    ...
    "proxy": "输入 train，可选 validation；主模型必须调用 QwenCausalProxy，让 CPU Qwen3-0.6B 本体直接按 A/B logits 打分。train 只选择 demonstrations，validation 只选阈值。禁止 Qwen embedding+LR 和静默 fallback。返回 artifacts.model。",
...
Proxy 必须调用 `okagent.qwen_causal_proxy.QwenCausalProxy`，由 CPU 上的 Qwen3-0.6B 因果语言模型本体直接比较 A/B token logits；禁止用 `qwen_proxy.load_qwen_features`、LogisticRegression、TF-IDF 或已有 embedding 代替主 proxy，禁止静默 fallback。
只把 train 标签交给 `fit` 选择少量平衡 demonstrations；validation 只用于阈值。先用 2 人验证 `score_ids`，再小 batch 全库评分，并用 `save` 保存 backend/model/config/demonstrations/threshold。
主动采样从 SemanticOperator('.').cached_labels() 的键排除全部已标注 ID，不能只用启动时的训练文件；读取行数组时提取 row['candidate_id']，不能把整个字典转成字符串当 ID。
Deploy 必须用 `QwenCausalProxy.load` 恢复同一模型、demonstrations 和阈值；自检同一 ID 在验证和全库路径中的 Qwen 分数一致。
